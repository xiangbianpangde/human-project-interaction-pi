...
| FR-004 | MachineResult 记录 verdict、输入、命令、环境、证据、hash、side effect、限制和未验证项 | **部分实现** | interaction v1 schema 保持冻结；companion contract 要求 PASS facts 非空、fact id 唯一且全部 VERIFIED；非 PASS 权威状态不可升格 | schema 已预留 execution/changes/side_effects，但当前 Adapter 尚不产出真实命令、环境、artifact SHA、side effects 或独立验证层 |
...
| FR-014 | NOT-RUN/INCOMPLETE/DEVIATIONS/BLOCKED、恢复路径和 Reconciler | **部分实现** | outbox v2 绑定 candidate digest；malformed entry 单条隔离；same event ID divergent digest 产生确定性 conflict 且不恢复；StaleReport 仍为 preview | 尚无 event-store Reconciler、half-commit 扫描、恢复事务或下游状态更新 |
...
| FR-022 | 长等待/人工待决恢复到相同 revision | **部分实现** | Pi session outbox 恢复、同 digest 去重、divergent identity conflict、source drift stale | 缺异步提醒、等待策略、跨设备 canonical request 和长期无人回答策略 |
