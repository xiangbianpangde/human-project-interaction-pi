...
- Add the append-only `.pi/artifacts/hpi-validation/v1/<attempt_id>` store with exclusive attempt locks, immutable content revisions, an isolated cwd-anchored write worker, fsync + atomic hard-link no-replace publication, strict record/result resolution, and one-Gate-one-fact PASS binding.
...
- Add validation-only MachineResult → restricted HPS/Human Brief projection and the `hpi_validation` preview/run/status tool; preview is zero-write, run cannot write outside the isolated attempt store, and runtime/status current results plus projection re-evaluate current Gates and fail closed on source drift/unavailability without mutating historical results.
...
- Record full-repository Sol job `2a80405f…`: VRS1 remained RELEASE and formal TS-001 remained `NOT-RUN`, while package 0.6.0 was initially BLOCKED on Issue #7 because uninstall/rollback could delete a concurrently substituted non-HPI pathname after stale ownership classification.
...
- Close Issue #7 at exact candidate `8393d3d…` / tree `cc359a8…`: independent review and Sol follow-up `38ed711f…` found P0/P1/P2 = 0 and gave package 0.6.0 RELEASE WITH OBSERVATIONS; PR #8 merged the same tree as `7ccbaab…`, exact-head CI `33487922214` and post-merge CI `33489305184` passed on Linux and Windows. VRS1 remains RELEASE while formal TS-001 remains `NOT-RUN` and complete P0 remains NOT READY.
